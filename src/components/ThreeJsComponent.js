  ...
  useEffect(() => {
    if (first === 1)
      ...
      THREE.TrackballControls = function (object, domElement) {
        ...
        this.handleResize = function (parentElement) {
          if (parentElement) {
            var box = parentElement.getBoundingClientRect();
            var d = parentElement.ownerDocument.documentElement;
        ...
        function keydown(event) {
          ...
          if (_state !== STATE.NONE) {
            ...
            event.keyCode === _this.keys[STATE.ZOOM] &&
            !_this.noZoom
          ) {
            _state = STATE.ZOOM;
    ...
    })(
      {
        1: [
          function (require, module, exports) {
            ...
            var bodyWidth = canvas.parentElement.clientWidth;
            var bodyHeight = canvas.parentElement.clientHeight;
            var fps = 60;
            var frameTime;
            ...
            var renderloop = function () {
              ...
              if (now - lastTimeRender > 1000 / fps) {
                ...
                // Move the light (for example, in a circular path)
                light.rad1 += 0.01; // Adjust the movement speed as needed
                light.setPosition(light.rad1, light.rad2);

                // Update the camera's rotation (if needed)
                camera.rotation += lastTimeRender = +new Date();
            ...
            var resizeRenderer = function () {
              bodyWidth = canvas.parentElement.clientWidth; // Assuming canvasParentElement is defined
              bodyHeight = canvas.parentElement.clientHeight; // Assuming canvasParentElement is defined
              renderer.setSize(bodyWidth, bodyHeight);
              camera.init(
                get.radian(190),
                get.radian(-20),
